...
interface ConfiguredSpotifyIdsProps {
  theme: any;
  styles: any;
  isMobile: boolean;
}

export function ConfiguredSpotifyIds({ theme, styles, isMobile }: ConfiguredSpotifyIdsProps) {
  const { configuredSpotifyIds } = useConfigState();
  const { addToQueue, loadingSpotifyId } = useJukeboxState();

  if (configuredSpotifyIds.length === 0) {
    return null;
  }
